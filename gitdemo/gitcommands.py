print("Git Commands Reference") 
# git log is to show the commit history of the repository.
# git status is to show the current status of the working directory and staging area.
# means to show new chenges and stages you have accept or not 
# git add. will add all new changes at once
# git add path to add specific file
# git commit -m "message" to commit the changes with a message 
# git push to push the changes to the remote repository
# git pull to pull the latest changes from the remote repository not locally in my computer
# git clone to clone a remote repository to your local machine
# git branch to show the current branches in the repository
# git branch branch_name to create a new branch
# git merge branch_name to merge a specific branch into the current branch
# git switch branch_name to switch to a specific branch
# 